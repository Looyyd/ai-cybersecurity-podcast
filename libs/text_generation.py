...
def json_headlines_to_prompt(headlines):
    ...
    for headline in headlines:
        titles_string += f"Headline {i}" + headline["title"] + "\n"
        i += 1
...
def create_podcast_context(podcast_number):
    ...
    podcast_number_prompt = f"This is episode {podcast_number} of the podcast. Today's date is {date.today().strftime('%B %d, %Y')}.\n"
...
def create_headlines_to_podcast_prompt(selected_headlines, podcast_number):
    ...
    for headline_index, headline in enumerate(selected_headlines):
        articles_string += (
            "Headline " + str(headline_index + 1) + ": " + headline["title"] + "\n"
        )
        for link_index, link in enumerate(headline["links"]):
            ...
            export_string_to_file(
                summarized_article_text,
                f"podcasts/podcast{podcast_number}/summaries/summary_{headline_index+1}_{link_index+1}.txt",
            )
...
"""\n'
    ...
    # create full prompt
    full_prompt = (
        task_prompt + podcast_context + music_prompt + format_prompt + articles_string
    )
    return full_prompt
...
def validate_podcast_script_format(podcast_script, n_headlines):
    ...
    for line in podcast_script.splitlines():
        if line.strip() != "":
            if (
                not line.startswith("#")
                and not line.startswith("John:")
                and not line.startswith("Jane:")
            ):
                validation_prompt += (
                    'FAIL. The line "'
                    + line
                    + '" does not start with a character name or a music marker.\n'
                )
                return False, validation_prompt
    # passed lines start
    validation_prompt += (
        "OK. All lines start with a character name or a music marker.\n"
    )
    ...
    validation_prompt += (
        "OK. The number of transitions is equal to the number of headlines.\n"
    )
    ...
    if average_exchange < wanted_exchange:
        validation_prompt += (
            "FAIL. The average number of exchanges per section is less than "
            + str(wanted_exchange)
            + ". Please add more exchanges in the dialogue.\n"
        )
        return False, validation_prompt
    # passed average exchange check
    validation_prompt += (
        "OK. The average number of exchanges per section is at least "
        + str(wanted_exchange)
        + ".\n"
    )
...
def create_podcast_title(podcast_number, podcast_script):
    prompt = (
        'You will be given a podcast script. You must write a podcast title that is relevant to the podcast script.\n\
            Example title: "Episode 1: Iranian Hackers, Apple\'s Zero-Day Flaws, and Ex-Employee Password Risks"\
                You will generate the title for Episode '
        + str(podcast_number)
        + ".\n\n"
        + "Here is the script:\n"
        + podcast_script
    )
    # add retries in case of random failure
    retries = 0
    max_retries = 3
...
def create_podcast_description(podcast_number, podcast_script, headlines):
    prompt = (
        "You will be given a podcast script. You must write a podcast description that is relevant to the podcast script.\n\
            Example description: \"In the debut episode of The Cybersecurity AI Daily, hosts Jane and John discuss the latest cybersecurity news.\
                ...
                You will generate the description for episode "
        + str(podcast_number)
        + ".\n\n"
        + "Here is the script:\n"
        + podcast_script
...
def create_podcast_keywords(podcast_number, podcast_script):
    prompt = (
        'You will be given a podcast script. You must write a list of comma separeted keywords that are relevant to the podcast script.\n\
            ...
            You will generate the keywords for episode '
        + str(podcast_number)
        + ".\n\n"
        + "Here is the script:\n"
        + podcast_script
    ...
    podcast_keywords, _ = gpt_complete_until_format(
        prompt, validate_keywords_format, max_retries=3
    )
